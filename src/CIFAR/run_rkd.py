...
import Models
import models.CustomResNet18 as CustomResNet18
...
def train(student, teacher, train_dataloader, alpha, distance_loss, angle_loss, optimizer, criterion, distance_loss, angle_loss, args, epoch):
    train_loss = AverageMeter()
    train_error = AverageMeter()

    RKDDistanceLoss = AverageMeter()
    RKDAngleLoss = AverageMeter()
    ...
    for step, (images, labels) in enumerate(train_dataloader):
        ...
        RKDDistanceLoss.update(distance_loss.item(), batch_size)
        RKDAngleLoss.update(angle_loss.item(), batch_size)
        ...
        s2 = ' - {:.2f}ms/step - distance_loss: {:.3f} - angle_loss: {:.3f} -  - train_loss: {:.3f} - train_acc: {:.3f}'.format(
             1000 * (time.time() - start), distance_loss.item(), angle_loss.item(), train_loss.val, 1-train_error.val)
...
def epoch_loop(student, teacher, train_set, test_set, args):
    ...
    # optimizer
    optimizer = torch.optim.SGD(params=student.parameters(), lr=args.lr, momentum=args.momentum,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        summary_writer.add_scalar('distance_loss', norm_dir_loss, epoch)
        summary_writer.add_scalar('angle_loss', div_loss, epoch)
...
if __name__ == "__main__":
    ...
    student.fc = nn.Linear(student_model.output_size, num_classes)

    teacher = models.resnet50(pretrained=True)  # Keep ResNet50 as it is
    teacher.fc = nn.Linear(teacher_model.fc.in_features, num_classes)
